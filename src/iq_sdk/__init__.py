...
[`Receiver`][iq_sdk.Receiver] reads I/Q data from a single receiver directory produced by the
[recording format](format.md). Pass the path to the `rx*` subdirectory and,
optionally, an `interval` controlling how many samples are returned per item.
...
[`Sensor`][abstract_dataloader.spec.Sensor] protocol, so items are accessed by integer index:
